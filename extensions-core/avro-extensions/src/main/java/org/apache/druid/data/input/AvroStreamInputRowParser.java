...
import org.apache.druid.data.input.avro.ZYSchemaRegistryBasedAvroBytesDecoder;
...
public class AvroStreamInputRowParser implements ByteBufferInputRowParser {
   ...
   public List<InputRow> parseBatch(ByteBuffer input) {
      if (avroBytesDecoder instanceof ZYSchemaRegistryBasedAvroBytesDecoder) {
         final List<GenericRecord> genericRecords = avroBytesDecoder.parseBatch(input);
         final List<InputRow> tmps = new ArrayList<>(genericRecords.size());
         for (GenericRecord genericRecord : genericRecords) {
            final List<InputRow> inputRows = AvroParsers.parseGenericRecord(genericRecord, mapParser, avroFlattener);
            tmps.addAll(inputRows);
         }
         return tmps;
      } else {
